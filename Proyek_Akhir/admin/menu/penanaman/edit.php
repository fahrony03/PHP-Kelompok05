...
if ($nama_file != "") {
    if($tipe_file == "image/jpeg" || $tipe_file == "image/png"){ // Cek apakah tipe file yang diupload adalah JPG / JPEG / PNG
        ...
        if($ukuran_file <= 1000000){ // Cek apakah ukuran file yang diupload kurang dari sama dengan 1MB
          ...
          if(move_uploaded_file($tmp_file, $path)){ // Cek apakah gambar berhasil diupload atau tidak
            ...
            $stmt = $pdo->prepare("UPDATE penanaman SET nama=:nama, slug=:slug, ukuran=:ukuran,tipe=:tipe,title=:title,content=:content WHERE id =:id" );
    ...
    $stmt = $pdo->prepare("UPDATE penanaman SET slug=:slug ,title=:title,content=:content WHERE id =:id");
    $stmt->bindParam(':slug',$slug);
    $stmt->bindParam(':title', $title);
    $stmt->bindParam(':content', $content);
    $stmt->bindParam(':id', $id);
